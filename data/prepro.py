...
def create_data_mats(data_toks, ques_inds, ans_inds, params, dtype):
    ...
    for i in range(num_threads):
        ...
        if dtype == 'test':
            image_list.append('%s2014/COCO_%s2014_%012d.jpg' %
                              ('val', 'val', image_id))
        else:
            image_list.append('%s2014/COCO_%s2014_%012d.jpg' %
                              ('train', 'train', image_id))
        image_index[i] = i
        caption_len[i] = len(data_toks[image_id]
                             ['caption_inds'][0:max_cap_len])
        captions[i][0:caption_len[i]
                    ] = data_toks[image_id]['caption_inds'][0:max_cap_len]
        ...
        for j in range(num_rounds):
            if data_toks[image_id]['dialog'][j]['question'] != -1:
                question_len[i][j] = len(
                    ques_inds[data_toks[image_id]['dialog'][j]['question']][0:max_ques_len])
                questions[i][j][0:question_len[i][j]] = ques_inds[data_toks[image_id]
                                                                  ['dialog'][j]['question']][0:max_ques_len]
            if data_toks[image_id]['dialog'][j]['answer'] != -1:
                answer_len[i][j] = len(
                    ans_inds[data_toks[image_id]['dialog'][j]['answer']][0:max_ans_len])
                answers[i][j][0:answer_len[i][j]] = ans_inds[data_toks[image_id]
    ...
    for i in range(num_threads):
        ...
        for j in range(num_rounds):
            options[i][j] = np.array(
                data_toks[image_id]['dialog'][j]['answer_options']) + 1
...
def split_train_val(data_train_val):
    ...
    '''
    data_train, data_val = copy.deepcopy(
        data_train_val), copy.deepcopy(data_train_val)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-download', default=0, type=int,
                        help='Whether to download VisDial v0.9 data')
    parser.add_argument('-version', default='0.5',
                        help='Choose the dataset version: 0.5 | 0.9', choices=['0.5', '0.9'])

    # Input files
    parser.add_argument(
        '-input_json_train', default='visdial_0.5_train.json', help='Input `train` json file')
    parser.add_argument(
        '-input_json_val', default='visdial_0.5_val.json', help='Input `val` json file')
    parser.add_argument(
        '-input_json_test', default='visdial_0.5_test.json', help='Input `test` json file')

    # Output files
    parser.add_argument(
        '-output_json', default='visdial/chat_processed_params.json', help='Output json file')
    parser.add_argument(
        '-output_h5', default='visdial/chat_processed_data.h5', help='Output hdf5 file')

    # Options
    parser.add_argument('-max_ques_len', default=20,
                        type=int, help='Max length of questions')
    parser.add_argument('-max_ans_len', default=20,
                        type=int, help='Max length of answers')
    parser.add_argument('-max_cap_len', default=40,
                        type=int, help='Max length of captions')
    ...
    if args.version == '0.5':
        print('Reading json...')
        data_train = map_format(
            json.load(open(args.input_json_train, 'r')), 'train')
        data_val = map_format(json.load(open(args.input_json_val, 'r')), 'val')
        data_test = map_format(
            json.load(open(args.input_json_test, 'r')), 'test')
    ...
    f.create_dataset('doc_train', dtype='uint32', data=captions_train)
    f.create_dataset('doc_length_train', dtype='uint32',
                     data=captions_train_len)
    ...
    f.create_dataset('summ_pos_train', dtype='uint32', data=images_train_index)
    ...
    f.create_dataset('doc_val', dtype='uint32', data=captions_val)
    f.create_dataset('doc_length_val', dtype='uint32', data=captions_val_len)
    ...
    f.create_dataset('summ_pos_val', dtype='uint32', data=images_val_index)
    ...
    out['unique_summ_train'] = images_train_list
    out['unique_summ_val'] = images_val_list
